...
module "initial" {
  ...
  project_vpc_zones                   = ["us-east-1a"]
  project_vpc_type                    = local.ip_family
  project_vpc_public                  = local.ip_family == "ipv6" ? false : true
  ...
  project_security_group_type         = (local.install_method == "rpm" ? "egress" : "project")
  ...
  project_domain_use_strategy         = "skip"
  project_domain                      = ""
  project_domain_zone                 = ""
  ...
  server_load_balancer_target_groups  = [local.k8s_target_group]
